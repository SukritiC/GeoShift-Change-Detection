...
def test_differencer():
    ...
    with rasterio.open(before_path) as src:
        profile = src.profile
        save_results(diff, mask, profile, "results/test_diff.tif", "results/test_mask.tif")
    
    if os.path.exists("results/test_diff.tif") and os.path.exists("results/test_mask.tif"):
