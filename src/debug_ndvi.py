import rasterio
import numpy as np
from src.differencer import calculate_ndvi
...
def debug_values():
    ...
    with rasterio.open(before_path) as src:
        img_before = src.read()
        print(
            f"Before Image Stats: Min={img_before.min()}, Max={img_before.max()}, Mean={img_before.mean()}"
        )
    ...
    with rasterio.open(after_path) as src:
        img_after = src.read()
        print(
            f"After Image Stats: Min={img_after.min()}, Max={img_after.max()}, Mean={img_after.mean()}"
        )

    # Calculate NDVI manually to check values
    ndvi_before = calculate_ndvi(img_before)
    print(
        f"NDVI Before Stats: Min={ndvi_before.min()}, Max={ndvi_before.max()}, Mean={ndvi_before.mean()}"
    )

    ndvi_after = calculate_ndvi(img_after)
    print(
        f"NDVI After Stats: Min={ndvi_after.min()}, Max={ndvi_after.max()}, Mean={ndvi_after.mean()}"
    )
