...
uploaded_before = st.sidebar.file_uploader("Upload 'Before' Image (GeoTIFF)", type=["tif", "tiff"])
uploaded_after = st.sidebar.file_uploader("Upload 'After' Image (GeoTIFF)", type=["tif", "tiff"])
...
if uploaded_before and uploaded_after:
    ...
    try:
        ...
        st.caption(f"**{pct_changed:.2f}%** of the total area has significant change (above threshold {threshold}).")
        ...
        col4.image(mask_display, caption="Change Mask (White = Change)", use_container_width=True)
